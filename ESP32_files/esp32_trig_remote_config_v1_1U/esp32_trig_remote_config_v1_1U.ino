...
bool inEventMode = false;
String activeEventPlaylist = "";
String activeEventDedupeKey = "";
...
String lastSevSent = "";
bool g_lastConfigApplyOk = true;
String g_lastConfigApplyError = "";
String g_lastConfigAppliedAt = "";
...
static void saveConfig() {
  ...
  d["playerId"] = cfg.playerId;
  d["playerName"] = cfg.playerName;
  ...
  d["lastConfigApplyOk"] = g_lastConfigApplyOk;
  d["lastConfigApplyError"] = g_lastConfigApplyError;
  d["lastConfigAppliedAt"] = g_lastConfigAppliedAt;
...
static void loadConfig() {
  ...
  cfg.playerId = (const char*)(d["playerId"] | cfg.playerId.c_str());
  cfg.playerName = (const char*)(d["playerName"] | cfg.playerName.c_str());
  ...
  g_lastConfigApplyOk = (bool)(d["lastConfigApplyOk"] | g_lastConfigApplyOk);
  g_lastConfigApplyError = (const char*)(d["lastConfigApplyError"] | g_lastConfigApplyError.c_str());
  g_lastConfigAppliedAt = (const char*)(d["lastConfigAppliedAt"] | g_lastConfigAppliedAt.c_str());
...
static bool looksHttpUrl(const String& s, bool allowEmpty = false) {
  String v = s;
  v.trim();
...
static void markConfigApplyResult(bool ok, const String& msg) {
  g_lastConfigApplyOk = ok;
  g_lastConfigApplyError = ok ? String("") : msg;
  g_lastConfigAppliedAt = String(millis());
}
...
static bool applyDesiredConfig(JsonObject desired, String& errMsg) {
  Config next = cfg;
  IPAddress ip;

  if (desired.containsKey("playerIp")) {
    String v = String((const char*)desired["playerIp"]);
    v.trim();
    if (v.length() && !ip.fromString(v)) {
      errMsg = "invalid playerIp";
      return false;
    }
    next.playerIp = v;
  }
  if (desired.containsKey("playerId")) next.playerId = String((const char*)desired["playerId"]);
  if (desired.containsKey("playerName")) next.playerName = String((const char*)desired["playerName"]);
  if (desired.containsKey("basicAuth")) next.basicAuth = String((const char*)desired["basicAuth"]);
  if (desired.containsKey("normalPlaylist")) next.normalPlaylist = String((const char*)desired["normalPlaylist"]);

  if (desired.containsKey("capUrl")) {
    String v = String((const char*)desired["capUrl"]);
    v.trim();
    if (!looksHttpUrl(v)) {
      errMsg = "invalid capUrl";
      return false;
    }
    next.capUrl = v;
  }
  if (desired.containsKey("deviceIso")) next.deviceIso = String((const char*)desired["deviceIso"]);

  if (desired.containsKey("pollSec")) next.pollSec = clampU16((long)desired["pollSec"], 5, 3600);
  if (desired.containsKey("cycles")) next.cycles = clampU8((long)desired["cycles"], 1, 10);
  if (desired.containsKey("eventPlaySec")) next.eventPlaySec = clampU16((long)desired["eventPlaySec"], 5, 36000);
  if (desired.containsKey("eventPlayMode")) next.eventPlayMode = clampU8((long)desired["eventPlayMode"], 0, 1);
  if (desired.containsKey("normalHoldSec")) next.normalHoldSec = clampU16((long)desired["normalHoldSec"], 5, 36000);

  if (desired.containsKey("useIdentifier")) next.useIdentifier = (bool)desired["useIdentifier"];
  if (desired.containsKey("allowStaleTrigger")) next.allowStaleTrigger = (bool)desired["allowStaleTrigger"];

  if (desired.containsKey("trigUrl")) {
    String v = String((const char*)desired["trigUrl"]);
    v.trim();
    if (!looksHttpUrl(v)) {
      errMsg = "invalid trigUrl";
      return false;
    }
    next.trigUrl = v;
  }
  if (desired.containsKey("trigToken")) next.trigToken = String((const char*)desired["trigToken"]);
  if (desired.containsKey("hbSec")) next.hbSec = clampU16((long)desired["hbSec"], 10, 3600);
  ...
  if (desired.containsKey("events") && desired["events"].is<JsonArray>()) {
    ...
    for (JsonObject o : arr) {
      ...
      if (o.containsKey("enabled")) next.evEnabled[idx] = (bool)o["enabled"];
      else if (o.containsKey("en")) next.evEnabled[idx] = (bool)o["en"];

      if (o.containsKey("playlist")) next.evPlaylist[idx] = String((const char*)o["playlist"]);
      else if (o.containsKey("pl")) next.evPlaylist[idx] = String((const char*)o["pl"]);

      if (o.containsKey("minRank")) next.evMinRank[idx] = clampU8((long)o["minRank"], 0, 4);
      else if (o.containsKey("min")) next.evMinRank[idx] = clampU8((long)o["min"], 0, 4);
    }
  }

  cfg = next;

  Serial.println("CFG: event mappings after apply:");
for (uint8_t i = 0; i < EVENT_COUNT; i++) {
  Serial.printf("  %s en=%u min=%u pl=%s\n",
                EVENT_DEFS[i].key,
                (unsigned)cfg.evEnabled[i],
                (unsigned)cfg.evMinRank[i],
                cfg.evPlaylist[i].c_str());
}
...
static void sendHeartbeat() {
  ...
  d["appliedConfigVersion"] = cfg.appliedConfigVersion;
  d["lastConfigApplyOk"] = g_lastConfigApplyOk;
  if (g_lastConfigApplyError.length()) d["lastConfigApplyError"] = g_lastConfigApplyError;
  if (g_lastConfigAppliedAt.length()) d["lastConfigAppliedAt"] = g_lastConfigAppliedAt;
  ...
  cfgSnap["playerId"] = cfg.playerId;
  cfgSnap["playerName"] = cfg.playerName;
  ...
  cfgSnap["hbSec"] = cfg.hbSec;
  cfgSnap["trigUrl"] = cfg.trigUrl;
  ...
  String applyErr;
  bool applied = applyDesiredConfig(desired, applyErr);
  markConfigApplyResult(applied, applyErr);
  if (!applied) {
    Serial.println("HB: config apply failed: " + applyErr);
    saveConfig();
    return;
  }
  ...
  saveConfig();
  Serial.println("HB: config apply success, version=" + String(cfg.appliedConfigVersion));
}
...
static void detectPlayerInfo() {
  Serial.println("Detecting player info...");

  IPAddress ip;
  if (!ip.fromString(cfg.playerIp)) {
    Serial.println("Player detect: invalid IP");
    return;
  }
  ...
  if (c.connect(ip, 8000)) {
    ...
    c.print("GET /api/device HTTP/1.1\r\n");
    c.print("Host: " + cfg.playerIp + "\r\n");
    c.print("Authorization: " + cfg.basicAuth + "\r\n");
    c.print("Connection: close\r\n\r\n");
    ...
    DeserializationError err = deserializeJson(d, c);

    if (!err) {
      cfg.playerId = String((const char*)d["data"]["deviceId"]);
      Serial.println("Player ID detected: " + cfg.playerId);
    }
  ...
  /* -------- GET /api/status (playerName) -------- */
  if (c.connect(ip, 8000)) {

    c.print("GET /api/status HTTP/1.1\r\n");
    c.print("Host: " + cfg.playerIp + "\r\n");
    c.print("Authorization: " + cfg.basicAuth + "\r\n");
    c.print("Connection: close\r\n\r\n");
    ...
    if (!err) {
      cfg.playerName = String((const char*)d["data"]["playerName"]);
      Serial.println("Player Name detected: " + cfg.playerName);
    }
...
void setup() {
  ...
  if (g_lastConfigAppliedAt.length() == 0) {
    g_lastConfigApplyOk = true;
  }
...
void loop() {
  ...
  if ((now - lastHbMs) >= (uint32_t)cfg.hbSec * 1000UL) {
   ...
   // ========= SIMPLE ALERT / NORMAL SWITCHING =========
  if ((now - lastPollMs) >= (uint32_t)cfg.pollSec * 1000UL) {
    lastPollMs = now;

    CapResult r = pollCapUrl(cfg.capUrl, "ndwc");
    if (!r.ok) {
      Serial.println("CAP: poll failed");
      return;
    }
